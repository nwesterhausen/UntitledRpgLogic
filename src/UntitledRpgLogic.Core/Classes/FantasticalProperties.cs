...
public record FantasticalProperties
{
	...
	private FantasticalProperties(float? aetherialConductivity, Dictionary<Guid, float>? elementalAttunement, float? manaCapacity,
	...
	public static FantasticalProperties FromConfig(FantasticalPropertiesConfig config)
	{
		...
		return new FantasticalProperties(config.AetherialConductivity, config.MagicalAttunement, config.ManaCapacity, config.Purity,
			config.Luminosity);
	}
}
