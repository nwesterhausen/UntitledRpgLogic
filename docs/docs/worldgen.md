...
When generating the various layers used in world gen, it follows a set order of generation, as some layers depend on others. Here is a breakdown of the content in the world map configuration file, and then the steps taken for generating a new world map.
...
Generates noise maps based only on a seed and noise generation settings: not influenced by other maps. All of these could be done at the same time (in parallel).
...
### Teir 1.5
These are slight adjustments to a Tier 1 based on another Tier 1 after all are generated. These involve reading at least one map and modifying another. Must   be serialized.
...
These depend on one-another or first tier noise maps to generate biased noise maps. These must be serialized. 
...
These only store information about the world grid based on information in lower tiers. These are descriptive maps based on values in other maps. 
...
### Tier 3.5: 
A final pass to allow magical influence on the world. This modifies some maps based on the magical elements present.
