...
public abstract partial class MinorStat(string name, int maxValue) : StatBase(StatVariation.Minor,name, maxValue) {
  /// <summary>
  ...
  public bool IsDependentOn(MajorStat majorStat) => _majorStats.ContainsKey(majorStat);
  ...
  public void Unlink(MajorStat majorStat) {
    if (!_majorStats.Remove(majorStat) || !_majorStatHandlers.TryGetValue(majorStat, out var handler)) {
      return;
    }
  ...
  public sealed override void AddPoints(int points) => LogIllegalStatChangeAttempt(Name, $"+{points}");
  ...
  public sealed override void SubtractPoints(int points) => LogIllegalStatChangeAttempt(Name, $"-{points}");
